        ...
        {loading ? (
          ...
          <>
            ...
            <ScrollView className='w-full p-4' contentContainerStyle={{
              ...
              <Text className='text-2xl font-bold mb-4'>Module 1: Essentials</Text>
            
              {lessons.map((lesson, index) => lesson !== undefined ? (
                
                <View key={lesson.rootPath} className={`items-center my-2 ${selectedBadge === index ? 'border-2 border-blue-500 rounded-lg' : ''}`}> 
                  <LessonSelectBadge  
                    {...lesson}
              ...
              <RaisedButton variant={'continue'} buttonStyles="p-4 mt-4" onPress={() => navigation.navigate('Lesson')}>
                <Text className='text-white'>Lesson On Demand</Text>
              </RaisedButton>
            </ScrollView>
          </>
        )}
